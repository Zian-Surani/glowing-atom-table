...
export const useElementData = (elementSymbol: string, atomicNumber: number) => {
  ...
  useEffect(() => {
    const fetchElementData = async () => {
      ...
      // Always set the detailed info from our database, regardless of API success
      setData({
        discoveryHistory: detailedInfo.discoveryHistory,
        physicalProperties: {
          appearance: detailedInfo.appearance,
          density: 'Loading from database...',
          hardness: 'Not specified',
          conductivity: 'Not specified',
        },
        chemicalProperties: {
          oxidationStates: 'Not available',
          ionizationEnergy: 'Not available',
          electronegativity: 'Not available',
          atomicRadius: 'Not available',
        },
      ...
      // Try to enhance with API data, but don't fail if it's unavailable
      try {
        ...
        if (response.ok) {
          ...
          // Update only the properties we got from the API
          setData(prev => ({
            ...prev,
            physicalProperties: {
            ...
            chemicalProperties: {
              ...
              oxidationStates: elementInfo.oxidationStates || prev.chemicalProperties.oxidationStates,
              ionizationEnergy: elementInfo.ionizationEnergy ? `${elementInfo.ionizationEnergy} eV` : prev.chemicalProperties.ionizationEnergy,
              electronegativity: elementInfo.electronegativity || prev.chemicalProperties.electronegativity,
              atomicRadius: elementInfo.atomicRadius ? `${elementInfo.atomicRadius} pm` : prev.chemicalProperties.atomicRadius,
            }
          }));
        }
      } catch (error) {
        console.log(`External API unavailable for ${elementSymbol}, using built-in data`);
